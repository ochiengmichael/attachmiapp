...
app.post('/api/auth/forgot-password', (req, res) => {
  ...
  res.json({ success: true, message: 'A secure password reset link has been successfully dispatched to your email.' });
});
...
app.post('/api/auth/reset-password', (req, res) => {
  ...
  if (!token || !newPassword) {
    res.status(400).json({ error: 'Token and replacement new password are required.' });
    return;
  }
