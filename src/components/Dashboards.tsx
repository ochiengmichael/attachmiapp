...
import { 
  ...
  Settings, MessageSquare, AlertCircle, RefreshCw, Eye
} from 'lucide-react';
    ...
    <div className="bg-transparent flex-grow py-8 relative z-10" id="dashboard-portal-view">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 rounded-3xl bg-[#0E0E14]/90 p-6 sm:p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center border border-white/10 shadow-2xl backdrop-blur-md">
          <div>
            <span className="text-[10px] uppercase tracking-widest text-indigo-400 font-bold font-mono">Secure Authorized Area</span>
            <h1 className="text-2xl sm:text-3xl mt-1 text-white font-display font-light tracking-wide">Hello, {user.name}</h1>
            <p className="text-xs text-slate-400 mt-1.5 max-w-2xl leading-relaxed">
              Welcome to your personal AttachME hub. You are authenticated with the role authorization level: <span className="font-bold text-indigo-400 capitalize">{user.role === 'job_seeker' ? 'general job seeker' : user.role === 'student' ? 'university student candidate' : user.role}</span>.
            </p>
          </div>
        ...
        {/* Dynamic Role routing rendering */}
